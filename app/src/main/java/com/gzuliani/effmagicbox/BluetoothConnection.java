...
public class BluetoothConnection {
    ...
    public BluetoothConnection(String address) throws RuntimeException {
    ...
    public byte[] receive(int maxBytesToRead, int timeout) throws IOException {
        Log.d("BluetoothConnection", String.format("receiving %d bytes in %dms...", maxBytesToRead,  timeout));

        int availableBytes = 0;
        int elapsed = 0;
        final int tick = 10; // ms

        while((availableBytes < maxBytesToRead) && (elapsed < timeout)) {
            elapsed += tick;
